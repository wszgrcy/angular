...
export class DefaultIterableDiffer<V> implements IterableDiffer<V>, IterableChanges<V> {
  ...
  constructor(/**默认传入是空*/trackByFn?: TrackByFunction<V>) {
    this._trackByFn = trackByFn || trackByIdentity;
    // console.log('trackByFn', this._trackByFn, '默认?', trackByIdentity, '注入?', trackByFn)
  }
  ...
  check(collection: NgIterable<V>): boolean {
    ...
    // console.log(this._additionsHead, this._movesHead,
    //   this._removalsHead, this._identityChangesHead)
    return this.isDirty;
  }
  ...
  _mismatch(record: IterableChangeRecord_<V> | null, item: V, itemTrackBy: any, index: number):
    ...
    console.log(record, item, itemTrackBy, index)
    // The previous record after which we will append the current one.
    let previousRecord: IterableChangeRecord_<V> | null;
    //doc 从尾部开始向前推?
    // console.log('尾部', this._itTail)
    if (record === null) {
  ...
  _unlink(record: IterableChangeRecord_<V>): IterableChangeRecord_<V> {
    console.log(this._linkedRecords)
    if (this._linkedRecords !== null) {
...
export class IterableChangeRecord_<V> implements IterableChangeRecord<V> {
  ...
  constructor(public item: V, public /**追踪值*/trackById: any) { }
}
