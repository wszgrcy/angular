...
export class CompileMetadataResolver {
  private _nonNormalizedDirectiveCache = new Map<
    Type,
  ...
  private _shallowModuleCache = new Map<
    Type,
    cpl.CompileShallowModuleMetadata
  >();
  ...
  private _createProxyClass(baseType: any, name: string): cpl.ProxyClass {
    ...
    const proxyClass: cpl.ProxyClass = <any>function(this: unknown) {
      if (!delegate) {
        throw new Error(
          `Illegal state: Class ${name} for type ${stringify(
            baseType
          )} is not compiled yet!`
        );
      }
  ...
  private getGeneratedClass(
    dirType: any,
    name: string
  ): StaticSymbol | cpl.ProxyClass {
    if (dirType instanceof StaticSymbol) {
      return this._staticSymbolCache.get(
        ngfactoryFilePath(dirType.filePath),
        name
      );
    } else {
  ...
  private getComponentFactory(
    ...
    if (dirType instanceof StaticSymbol) {
      return this._staticSymbolCache.get(
        ngfactoryFilePath(dirType.filePath),
        cpl.componentFactoryName(dirType)
      );
    } else {
      const hostView = this.getHostComponentViewClass(dirType);
      // Note: ngContentSelectors will be filled later once the template is
      // loaded.
      const createComponentFactory = this._reflector.resolveExternalReference(
  ...
  private _loadSummary(
    type: any,
    kind: cpl.CompileSummaryKind
  ): cpl.CompileTypeSummary | null {
    //doc 查看缓存
  ...
  getHostComponentMetadata(
    ...
    const template = CssSelector.parse(
      compMeta.selector!
  ...
  loadDirectiveMetadata(
    ...
    const { annotation, metadata } = this.getNonNormalizedDirectiveMetadata(
      directiveType
    )!;

    const createDirectiveMetadata = (
      templateMetadata: cpl.CompileTemplateMetadata | null
    ) => {
    ...
    if (metadata.isComponent) {
      ...
      const templateMeta = this._directiveNormalizer.normalizeTemplate({
        ...
        moduleUrl: this._reflector.componentModuleUrl(
          directiveType,
          annotation
        ),
      ...
      if (isPromise(templateMeta) && isSync) {
        this._reportError(
          componentStillLoadingError(directiveType),
          directiveType
        );
        return null;
      }
  ...
  getNonNormalizedDirectiveMetadata(
    ...
    if (createComponent.isTypeOf(dirMeta)) {
      ...
      if (compMeta.viewProviders) {
        viewProviders = this._getProvidersMetadata(
          compMeta.viewProviders,
          entryComponentMetadata,
          `viewProviders for "${stringifyType(directiveType)}"`,
          [],
          directiveType
        );
      }
    ...
    if (dirMeta.providers != null) {
      providers = this._getProvidersMetadata(
        dirMeta.providers,
        entryComponentMetadata,
        `providers for "${stringifyType(directiveType)}"`,
        [],
        directiveType
      );
    }
    ...
    if (dirMeta.queries != null) {
      ...
      viewQueries = this._getQueriesMetadata(
        dirMeta.queries,
        true,
        directiveType
      );
    ...
    const metadata = cpl.CompileDirectiveMetadata.create({
      ...
      rendererType: nonNormalizedTemplateMetadata
        ? this.getRendererType(directiveType)
        : null,
      componentFactory: null
    });
    if (nonNormalizedTemplateMetadata) {
      metadata.componentFactory = this.getComponentFactory(
        selector,
        directiveType,
        metadata.inputs,
        metadata.outputs
  ...
  getDirectiveMetadata(directiveType: any): cpl.CompileDirectiveMetadata {
    ...
    if (!dirMeta) {
      this._reportError(
        syntaxError(
          `Illegal state: getDirectiveMetadata can only be called after loadNgModuleDirectiveAndPipeMetadata for a module that declares it. Directive ${stringifyType(
            directiveType
          )}.`
        ),
        directiveType
  ...
  getDirectiveSummary(dirType: any): cpl.CompileDirectiveSummary {
    const dirSummary = <cpl.CompileDirectiveSummary>(
      this._loadSummary(dirType, cpl.CompileSummaryKind.Directive)
    );
    if (!dirSummary) {
      this._reportError(
        syntaxError(
          `Illegal state: Could not load the summary for directive ${stringifyType(
            dirType
          )}.`
        ),
        dirType
  ...
  isDirective(type: any) {
    return (
      !!this._loadSummary(type, cpl.CompileSummaryKind.Directive) ||
  ...
  isPipe(type: any) {
    return (
      !!this._loadSummary(type, cpl.CompileSummaryKind.Pipe) ||
  ...
  isNgModule(type: any) {
    return (
      !!this._loadSummary(type, cpl.CompileSummaryKind.NgModule) ||
  ...
  getNgModuleSummary(
    ...
    let moduleSummary: cpl.CompileNgModuleSummary | null = <
      cpl.CompileNgModuleSummary
    >this._loadSummary(moduleType, cpl.CompileSummaryKind.NgModule);
    //doc 走下面的支线赋值是从递归中拿到
    if (!moduleSummary) {
      const moduleMeta = this.getNgModuleMetadata(
        moduleType,
        false,
        alreadyCollecting
      );
  ...
  loadNgModuleDirectiveAndPipeMetadata(
    ...
    if (ngModule) {
      ngModule.declaredDirectives.forEach(id => {
        const promise = this.loadDirectiveMetadata(
          moduleType,
          id.reference,
          isSync
        );
      ...
      ngModule.declaredPipes.forEach(id =>
        this._loadPipeMetadata(id.reference)
      );
    }
  ...
  getShallowModuleMetadata(
    ...
    const ngModuleMeta = findLast(
      this._reflector.shallowAnnotations(moduleType),
  ...
  getNgModuleMetadata(
    ...
    if (meta.imports) {
      flattenAndDedupeArray(meta.imports).forEach(importedType => {
        console.log("应该是imports的模块", importedType);
        /**imports里面的的类 */
        let importedModuleType: Type = undefined!;
        console.log("只要是函数就是true", isValidType(importedType));
          ...
          if (moduleWithProviders.providers) {
            ...
            providers.push(
              ...this._getProvidersMetadata(
                ...
                entryComponents,
                `provider for the NgModule '${stringifyType(
                  importedModuleType
                )}'`,
                [],
                importedType
        ...
        if (importedModuleType) {
          ...
          console.log("应该没有", alreadyCollecting);
          if (alreadyCollecting.has(importedModuleType)) {
            this._reportError(
              syntaxError(
                `${this._getTypeDescriptor(
                  importedModuleType
                )} '${stringifyType(
                  importedType
                )}' is imported recursively by the module '${stringifyType(
                  moduleType
                )}'.`
              ),
              moduleType
          ...
          //doing 进入1
          const importedModuleSummary = this.getNgModuleSummary(
            importedModuleType,
            alreadyCollecting
          );
          ...
          if (!importedModuleSummary) {
            this._reportError(
              syntaxError(
                `Unexpected ${this._getTypeDescriptor(
                  importedType
                )} '${stringifyType(
                  importedType
                )}' imported by the module '${stringifyType(
                  moduleType
          ...
          this._reportError(
            syntaxError(
              `Unexpected value '${stringifyType(
                importedType
    ...
    if (meta.exports) {
      flattenAndDedupeArray(meta.exports).forEach(exportedType => {
        ...
        if (alreadyCollecting.has(exportedType)) {
          this._reportError(
            syntaxError(
              `${this._getTypeDescriptor(exportedType)} '${stringify(
                exportedType
              )}' is exported recursively by the module '${stringifyType(
                moduleType
              )}'`
            ),
            moduleType
        ...
        const exportedModuleSummary = this.getNgModuleSummary(
          exportedType,
          alreadyCollecting
        );
        ...
        if (exportedModuleSummary) {
          ...
          exportedNonModuleIdentifiers.push(
            //doc 返回一个解析(向前引用)好 的{ reference: type }
            this._getIdentifierMetadata(exportedType)
          );
        }
      });
    }
    ...
    const transitiveModule = this._getTransitiveNgModuleMetadata(
      importedModules,
      exportedModules
    );
          ...
          this._reportError(
            syntaxError(
              `Unexpected ${this._getTypeDescriptor(
                ...
                declaredType
              )}' declared by the module '${stringifyType(
                moduleType
        ...
        this._reportError(
          syntaxError(
            `Can't export ${this._getTypeDescriptor(
              exportedId.reference
            )} ${stringifyType(exportedId.reference)} from ${stringifyType(
              moduleType
    ...
    if (meta.providers) {
      ...
      providers.push(
        ...this._getProvidersMetadata(
          meta.providers,
          entryComponents,
          `provider for the NgModule '${stringifyType(moduleType)}'`,
          [],
          moduleType
        )
      );
    ...
    if (meta.entryComponents) {
      entryComponents.push(
        ...flattenAndDedupeArray(meta.entryComponents).map(
          type => this._getEntryComponentMetadata(type)!
        )
      );
    }
        ...
        if (!isValidType(type)) {
          this._reportError(
            syntaxError(
              `Unexpected value '${stringifyType(
                type
              )}' used in the bootstrap property of module '${stringifyType(
                moduleType
              )}'`
            ),
            moduleType
    ...
    entryComponents.push(
      ...bootstrapComponents.map(
        type => this._getEntryComponentMetadata(type.reference)!
      )
    );
    ...
    providers.forEach(provider =>
      transitiveModule.addProvider(provider, compileMeta!.type)
    );
  ...
  private _checkSelfImport(
    moduleType: Type,
    importedModuleType: Type
  ): boolean {
    if (moduleType === importedModuleType) {
      this._reportError(
        syntaxError(
          `'${stringifyType(moduleType)}' module can't import itself`
        ),
        moduleType
      );
      return true;
    ...
    if (oldModule && oldModule !== moduleType) {
      this._reportError(
        syntaxError(
          `Type ${stringifyType(
            type
          )} is part of the declarations of 2 modules: ${stringifyType(
            oldModule
          )} and ${stringifyType(moduleType)}! ` +
            `Please consider moving ${stringifyType(
              type
            )} to a higher module that imports ${stringifyType(
              oldModule
            )} and ${stringifyType(moduleType)}. ` +
            `You can also create a new NgModule that exports and includes ${stringifyType(
              type
            )} then import that NgModule in ${stringifyType(
              oldModule
            )} and ${stringifyType(moduleType)}.`
        ),
        moduleType
...
/**
 * doing 
 * @returns 返回一个有各种处理过的装饰器参数{模块,入口组件,提供者,指令,管道,导出指令,导出管道}
 */
  private _getTransitiveNgModuleMetadata(
    importedModules: cpl.CompileNgModuleSummary[],
    exportedModules: cpl.CompileNgModuleSummary[]
  ): cpl.TransitiveCompileNgModuleMetadata {
    ...
    exportedModules.forEach(modSummary => {
      modSummary.exportedDirectives.forEach(id =>
        result.addExportedDirective(id)
      );
...
/**
 * 将模块返回为{ reference: type },解析type的向前引用
 */
  ...
  getInjectableMetadata(
    ...
    const typeSummary = this._loadSummary(
      type,
      cpl.CompileSummaryKind.Injectable
    );
    const typeMetadata = typeSummary
      ? typeSummary.type
      : this._getTypeMetadata(type, dependencies, throwOnUnknownDeps);

    const annotations: Injectable[] = this._reflector
      .annotations(type)
  ...
  private _getTypeMetadata(
    ...
    return {
      ...
      lifecycleHooks: getAllLifecycleHooks(
        this._reflector,
        identifier.reference
      )
    };
  ...
  getPipeMetadata(pipeType: any): cpl.CompilePipeMetadata | null {
    ...
    if (!pipeMeta) {
      this._reportError(
        syntaxError(
          `Illegal state: getPipeMetadata can only be called after loadNgModuleDirectiveAndPipeMetadata for a module that declares it. Pipe ${stringifyType(
            pipeType
          )}.`
        ),
        pipeType
  ...
  getPipeSummary(pipeType: any): cpl.CompilePipeSummary {
    const pipeSummary = <cpl.CompilePipeSummary>(
      this._loadSummary(pipeType, cpl.CompileSummaryKind.Pipe)
    );
    if (!pipeSummary) {
      this._reportError(
        syntaxError(
          `Illegal state: Could not load the summary for pipe ${stringifyType(
            pipeType
          )}.`
        ),
        pipeType
  ...
  private _getDependenciesMetadata(
    ...
    const dependenciesMetadata: cpl.CompileDiDependencyMetadata[] = params.map(
      param => {
    ...
    if (hasUnknownDeps) {
      const depsTokens = dependenciesMetadata
        .map(dep => (dep.token ? stringifyType(dep.token) : "?"))
        .join(", ");
      const message = `Can't resolve all parameters for ${stringifyType(
        typeOrFunc
      )}: (${depsTokens}).`;
      if (throwOnUnknownDeps || this._config.strictInjectionParameters) {
        ...
        this._console.warn(
          `Warning: ${message} This will become an error in Angular v6.x`
        );
      }
    }
  ...
  private _getProvidersMetadata(
    providers: Provider[],
    targetEntryComponents: cpl.CompileEntryComponentMetadata[],
    debugInfo?: string,
    compileProviders: cpl.CompileProviderMetadata[] = [],
    type?: any
  ): cpl.CompileProviderMetadata[] {
    providers.forEach((provider: any, providerIdx: number) => {
      if (Array.isArray(provider)) {
        this._getProvidersMetadata(
          provider,
          targetEntryComponents,
          debugInfo,
          compileProviders
        );
      } else {
        ...
        if (
          ...
          const providersInfo = (<string[]>providers.reduce(
        ...
        if (
          ...
          targetEntryComponents.push(
            ...this._getEntryComponentsFromProvider(providerMeta, type)
          );
        } else {
  ...
  private _validateProvider(provider: any): void {
    if (provider.hasOwnProperty("useClass") && provider.useClass == null) {
      this._reportError(
        syntaxError(
          `Invalid provider for ${stringifyType(
            provider.provide
  ...
  private _getEntryComponentsFromProvider(
    ...
    collectedIdentifiers.forEach(identifier => {
      const entry = this._getEntryComponentMetadata(
        identifier.reference,
        false
      );
      if (entry) {
  ...
  private _getEntryComponentMetadata(
    ...
    if (throwIfNotFound) {
      throw syntaxError(
        `${dirType.name} cannot be used as an entry component.`
      );
    }
    return null;
  }
  ...
  getProviderMetadata(provider: cpl.ProviderMeta): cpl.CompileProviderMetadata {
    ...
    let token: cpl.CompileTokenMetadata = this._getTokenMetadata(
      provider.token
    );

    if (provider.useClass) {
      compileTypeMetadata = this._getInjectableTypeMetadata(
        provider.useClass,
    ...
    return {
      ...
      useExisting: provider.useExisting
        ? this._getTokenMetadata(provider.useExisting)
        : undefined,
      deps: compileDeps,
  ...
  private _getQueryMetadata(
    ...
    if (typeof q.selector === "string") {
      selectors = this._queryVarBindings(q.selector).map(varName =>
        this._getTokenMetadata(varName)
      );
    } else {
      if (!q.selector) {
        this._reportError(
          syntaxError(
            `Can't construct a query for the property "${propertyName}" of "${stringifyType(
              typeOrFunc
...
function extractIdentifiers(
  value: any,
  targetIdentifiers: cpl.CompileIdentifierMetadata[]
) {
